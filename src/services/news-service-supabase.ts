...
export class NewsServiceSupabase {
  ...
  static async getNewsById(idOrSlug: string): Promise<NewsPost | null> {
    try {
      ...
      let { data: newsItem, error } = await supabase
        .from("patch_notes")
        .select("*")
        .eq("id", idOrSlug)
        .single();
      ...
      if (!newsItem) {
        ...
        if (slugError) {
          console.error("Error getting news by slug:", slugError);
        } else {
          newsItem = data;
        }
      }

      return newsItem ? processNewsContent(newsItem as NewsPost) : null;
    } catch (error) {
  ...
  static async getPatchNotes(params: NewsQueryParams = {}): Promise<PaginatedResponse<NewsPost>> {
    try {
      ...
      if (supabase) {
        ...
        try {
          // Calculate pagination for Supabase query
