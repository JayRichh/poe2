...
class ItemsService {
  ...
  private indexData() {
    ...
    for (const [groupName, categories] of Object.entries(this.itemsData as AllItemData)) {
      ...
      for (const [categoryName, category] of Object.entries(categories)) {
        ...
        if (slot) {
          console.log(`Indexing ${typedCategory.items.length} items from ${categoryName} to slot ${slot}`);
  ...
  searchItems(query: string): ItemBase[] {
    ...
    console.log('Items by slot:', Array.from(this.itemsBySlot.entries()).map(([slot, items]) => 
      ...
      items.forEach(item => {
        const matches = 
          ...
          (item.grantedSkill && (
            ...
            (item.grantedSkill.description && searchRegex.test(item.grantedSkill.description))
          )) ||
