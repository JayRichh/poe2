...
export function GlobalSettingsPanel({ settings, onChange }: GlobalSettingsPanelProps) {
  const handleSliderChange = (key: keyof GlobalSettings, value: number, min: number, max: number, step = 1) => {
  ...
  const renderNumericSetting = ({ key, label, min, max, step = 1, tooltip, formatValue }: NumericSetting) => {
  ...
  const toggleSettings: ToggleSetting[] = [
    ...
    { key: 'electrocution', label: 'Electrocution', tooltip: '25% increased damage to shocked enemies' },
    ...
    <Card>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 divide-x divide-border/60">
        <div key="attack" className="px-4 py-4">
          <Text className="text-sm font-medium text-foreground-secondary mb-6">Attack Settings</Text>
        ...
        <div key="crit" className="px-4 py-4">
          <Text className="text-sm font-medium text-foreground-secondary mb-6">Critical Strike</Text>
          <div className="space-y-6">
