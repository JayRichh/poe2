...
export function GuideCard({ title, description, sections }: GuideCardProps) {
  return (
    <div className="bg-card rounded-lg border border-border/50 overflow-hidden hover:border-border/80 transition-colors">
      <div className="p-8">
        <Text variant="h3" className="mb-3 text-2xl">{title}</Text>
        {description && (
        ...
        <div className="space-y-8">
          {sections.map((section, index) => (
            <div key={index}>
              <Text variant="h4" className="mb-4 text-xl font-semibold">{section.title}</Text>
              <ul className="list-disc list-outside space-y-3 ml-6">
                {section.tips.map((tip, tipIndex) => (
                  <li key={tipIndex} className="text-foreground/90 leading-relaxed break-words whitespace-normal">
                    {tip}
                  </li>
                ))}
              </ul>
